/**
 * @generated-file-note
 * 文件：smart-tools-backend/smart-tools-domain/src/main/java/com/example/smarttools/domain/admin/SysModuleRepository.java
 * 用途：后端数据访问层（Repository，基于 Spring Data JPA）
 * 归属：后端 smart-tools-domain
 * 分层：domain
 * 类型：interface SysModuleRepository
...
public interface SysModuleRepository extends JpaRepository<SysModule, Long> {
    ...
    List<SysModule> findAllByEnabledTrueOrderBySortOrderAscNameAsc();
}

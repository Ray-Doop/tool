/**
 * @generated-file-note
 * 文件：smart-tools-backend/smart-tools-auth/src/main/java/com/example/smarttools/modules/auth/domain/dto/OtpSendRequest.java
 * 用途：后端 Java 源码文件（Spring Boot 模块）
 * 归属：后端 smart-tools-auth
 * 分层：domain
 * 类型：record OtpSendRequest
 * 注解：NotBlank、Pattern
...
public record OtpSendRequest(
        ...
        // 发送场景（注册/登录）
        @Pattern(regexp = "^(register|login)$") String scene,
        // 可选图形验证码 ID
        String captchaId,
        // 可选图形验证码内容
        String captchaCode

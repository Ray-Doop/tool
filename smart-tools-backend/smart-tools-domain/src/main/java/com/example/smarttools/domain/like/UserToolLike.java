/**
 * @generated-file-note
 * 文件：smart-tools-backend/smart-tools-domain/src/main/java/com/example/smarttools/domain/like/UserToolLike.java
 * 用途：后端领域实体（JPA Entity，与数据库表映射）
 * 归属：后端 smart-tools-domain
 * 分层：domain
 * 类型：class UserToolLike
...
@Table(
        name = "user_tool_like",
        uniqueConstraints = @UniqueConstraint(name = "uk_user_tool_like", columnNames = {"user_id", "tool_slug"})
)
public class UserToolLike {
    @Id

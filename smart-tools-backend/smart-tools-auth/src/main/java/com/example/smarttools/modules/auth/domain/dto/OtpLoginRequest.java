/**
 * @generated-file-note
 * 文件：smart-tools-backend/smart-tools-auth/src/main/java/com/example/smarttools/modules/auth/domain/dto/OtpLoginRequest.java
 * 用途：后端 Java 源码文件（Spring Boot 模块）
 * 归属：后端 smart-tools-auth
 * 分层：domain
 * 类型：record OtpLoginRequest
 * 注解：NotBlank、Size
...
public record OtpLoginRequest(
        ...
        // 可选图形验证码 ID
        @Size(min = 8, max = 64) String captchaId,
        // 可选图形验证码内容
        @Size(min = 4, max = 8) String captchaCode
) {
}
